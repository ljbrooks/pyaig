...
class AGuessGate:
    def __init__(self, aiger, rootx=None):
        ...
        if rootx is None:
            self.rootx = list(aiger.get_po_fanins())
    ...
    def is_xor(self, i):
        return len(self.gatex[var(i)]) > 0 and isinstance(
            self.gatex[var(i)][0], AGate_XOR
        )
    ...
    def get_gate(self, i, gate_type, force=True):
        f = self.gatex[var(i)]
        if not force and not f:
            return None
        if not isinstance(f, gate_type) and not force:
            return None
    ...
    def toDiGraph(self):
        ...
        for i in self.acc.topox:
            ...
            name = self.get_id_name(
                i
            )  # self.aiger._id_to_name[i] if  i in self.aiger._id_to_name else str(var(i))
    ...
    def toDot(self, fname):
        ...
        pix = [
            str(i // 2)
            for i in self.acc.nodex
            if self.acc.levelx[var(i)] == 1 and i not in poS and self.marked[var(i)]
        ]
        k = k | set(pix)
        for i in pix:
            S.add_node(pydot.Node(i))
    ...
    def extend_and(self, lit):
        ...
        if len(kkx) > 2:
            ...
            if len(kkx) == 3:
                ...
                if not r is None:
                    # assert False
                    self.gatex[var(lit)] = [r] + self.gatex[
                        var(lit)
                    ]  # majority is first one
                    pass
                pass
            pass
        pass
    ...
    def identify_xor(self, lit):
        ...
        if not G is None:
            ...
            print(
                "found ",
                var(lit),
                "%s = %s %s %s" % (var(lit), lstr(xor[0]), G.name, lstr(xor[1])),
            )
            self.print_gate(lit)
