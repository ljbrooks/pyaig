...
class AigerProfile:
    ...
    def profile_first_level_maj(self):
        ...
        majx = list(
            filter(
                lambda i: self.ag.is_gate(i, AGate_Majority3)
                and self.acc.levelx[var(i)] <= 4,
                self.acc.topox,
            )
        )
    ...
    def print_maj3(self):
        for i in filter(
            ...
            print(
                f"{var(i)} is maj(%s)" % (",".join(map(lstr, g))),
                "level is",
                self.acc.levelx[var(i)],
            )
            print(
                f"{var(i)} is maj(%s)" % (",".join(map(self.ssymbol, g))),
                "level is",
                self.acc.levelx[var(i)],
            )
            print(
                f"{var(i)} is maj(%s)" % (",".join(map(self.get_pp, g))),
                "level is",
                self.acc.levelx[var(i)],
            )
            pass
