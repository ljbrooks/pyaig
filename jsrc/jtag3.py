...
def SetPrefix(p):  jtagCfg.PREFIX=p
...
def jtag(tag_, msg_, bs = -1 , debug_  = False, noskip = False):
    ...
    if tag_!= None:
        ...
        ret .append('%-60s%20s'% ( dmsg( TagIt(tag_) )+":" , '%s......'% (getCallser(bs))))
        print(ret [-1])
...
def hello(bs):
    ...
    (filename, line_number, function_name, lines, index) = inspect.getframeinfo(previous_frame)
    cur_fname = inspect.getframeinfo(cur)[0]
    print(  "## calling %s:%s from "% (cur_fname, inspect.getframeinfo(cur)[2]) , "%s:%d:%s" % (filename, line_number, function_name))
