...
class AColorRewrite:
    ...
    def dup_gate(self, i):
        if not self.v2new[var(i)] is None:
            return self.v2new[var(i)]
    ...
    def create_new(self):
        ...
        pi_name = (
            lambda i: self.aiger.get_name_by_id(i)
        ...
        for i in map(pure, filter(lambda v: self.marked[var(v)], self.topox)):
            ...
            pass
        # pdb.set_trace()
        for (po_id, po_fanin, po_type), po_name in zip(
            self.aiger.get_pos(), self.aiger.iter_po_names()
        ):
            print("map po", po_fanin, "to", self.map_lit(po_fanin))
            s = self.new_aiger.create_po(self.map_lit(po_fanin), po_name[-1], po_type)
            assert s == po_id
            pass
        pass
